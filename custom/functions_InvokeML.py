    ...
    def invoke_model(self, df, wml_endpoint, uid, password, instance_id, deployment_id, apikey, input_columns=[]):
        ...
        if 200 != response.status_code:
            ...
            if (len(input_columns) == 1):
                ...
                s_df = df
            # eSolutions URL Incorrect for V4
            # Payload change
            # payload = '{"input_data": [{"fields": %s,"values":%s}]}' %(json.dumps(input_columns), rows)
            # payload = json.dumps('{"input_data": [{"fields": %s,"values":%s}]}' % (json.dumps(input_columns), rows))

            payload = {"input_data": [{"fields": input_columns,
                                       "values": rows}]}
            ...
            wml_model_endpoint = '%s/ml/v4/deployments/%s/predictions?version=2021-03-31' % (wml_endpoint, deployment_id)
